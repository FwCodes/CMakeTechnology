...
//    protected static final int VOLUME_FUNC_NONE = 3;
    ...
    protected static List<Area> mMeteringAreaMatrix5 = new ArrayList<>();
    protected static List<Area> mMeteringAreaMatrix4 = new ArrayList<>();
    protected static List<Area> mMeteringAreaMatrix1 = new ArrayList<>();
    protected static List<Area> mMeteringAreaCenter = new ArrayList<>();
    protected static List<Area> mMeteringAreaSpot = new ArrayList<>();
            ...
            if (this.getIntent().getAction().equals(MediaStore.ACTION_IMAGE_CAPTURE)) {
                try {
                    if(this.getIntent().getExtras()!=null){
                        ...
                        if (forceFilenameUri != null) {
                            ApplicationScreen.setForceFilename(new File(forceFilenameUri.getPath()));
                        }else{
                    ...
                    if (ApplicationScreen.getForceFilename()!=null&&
                            ApplicationScreen.getForceFilename().getAbsolutePath().equals("/scrapSpace")) {
    ...
    protected void onApplicationStart() {
        ...
        if (cameraSelected == CameraController.getNumberOfCameras() - 1) {
            prefs.edit().putInt(ApplicationScreen.sCameraModePref, 0).apply();
...
//	protected CountDownTimer	onResumeTimer	= null;
    ...
    public void setSceneModePref(int iSceneMode) {
        PreferenceManager.getDefaultSharedPreferences(mainContext).edit()
                .putInt(ApplicationScreen.sSceneModePref, iSceneMode).apply();
    }
    ...
    public void setFocusModePref(int iFocusMode) {
        ...
        if (!CameraController.isRemoteCamera()) {
            PreferenceManager.getDefaultSharedPreferences(mainContext).edit()
                    .putInt(CameraController.isFrontCamera() ? frontFocusMode : backFocusMode, iFocusMode).apply();
        } else {
            // Sony cmaera
        }
    }
    ...
    public void setFlashModePref(int iFlashMode) {
        PreferenceManager.getDefaultSharedPreferences(mainContext).edit()
                .putInt(ApplicationScreen.sFlashModePref, iFlashMode).apply();
    }
