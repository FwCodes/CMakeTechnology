...
public class CameraController implements Camera.PictureCallback, AutoFocusCallback, Camera.ErrorCallback,
                    ...
                    if (camera != null && camera.getParameters() != null) {
                        ...
                        if (list != null) {
                            for (Camera.Size sz : list)
                                CameraController.SupportedPreviewSizesList.add(new Size(sz.width,
                                        sz.height));
                        }
                    }
                } catch (Exception e) {
            ...
            if (camera != null) {
                try {
                    ...
                    if (params != null) {
                        ...
                        for (Camera.Size sz : list)
                            CameraController.SupportedPictureSizesList.add(new Size(sz.width, sz.height));
    ...
    public static void populateCameraDimensionsSonyRemote() {
        ...
        int iHighestIndex = 0;
        Size sHighest = SonyRemoteCamera.getPictureSizeListRemote().get(0);

        for (int ii = 0; ii < SonyRemoteCamera.getPictureSizeListRemote().size(); ++ii) {
            Size s = SonyRemoteCamera.getPictureSizeListRemote().get(ii);
        ...
        if (CameraController.ResolutionsNamesList.isEmpty()) {
            Size s = SonyRemoteCamera.getPictureSizeListRemote().get(iHighestIndex);
    ...
    protected static void addResolution(int ii, int currSizeWidth, int currSizeHeight) {
        ...
        if (needAdd) {
            ...
            CameraController.ResolutionsSizeList.add(loc, new Size(currSizeWidth, currSizeHeight));
        }
    }
    ...
    protected static void fillResolutionsListMultishot(int ii, int currSizeWidth, int currSizeHeight) {
        ...
        if (needAdd) {
            ...
            CameraController.MultishotResolutionsSizeList.add(loc, new Size(currSizeWidth,
                    currSizeHeight));
        }
    }
        ...
        if (!CameraController.isRemoteCamera()) {
            ...
            if (!CameraController.isCamera2) {
                if (CameraController.SupportedPreviewSizesList != null) {
                    ...
                    for (Size sz : sizes)
                        previewSizes.add(new Size(sz.getWidth(), sz.getHeight()));
                } else {
    ...
    public static void setCameraPreviewSize(Size sz) {
            ...
            if (!CameraController.isCamera2) {
                if (CameraController.SupportedPictureSizesList != null) {
                    ...
                    for (Camera.Size sz : sizes)
                        pictureSizes.add(new Size(sz.width, sz.height));
                } else {
    ...
    public static List<Size> getSupportedVideoSizes() {
        ...
        if (!CameraController.isCamera2) {
            if (camera != null && camera.getParameters() != null) {
                ...
                for (Camera.Size sz : sizes)
                    videoSizes.add(new Size(sz.width, sz.height));
            } else {
                Log.d(TAG, "camera == null");
            }
        } else
        ...
        if ((activityManager.getMemoryClass() >= 128 || activityManager.getLargeMemoryClass() >= 512)
                ...
                && !videoSizes.contains(new Size(3840, 2160))))  //If device already supports 4K resolution skip adding
        {
            videoSizes.add(new Size(3840, 2160));
        }
